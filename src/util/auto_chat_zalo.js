                ...
                for (let content of chatContent) {
                    ...
                    if (lastestResponse.length > 0) {
                        ...
                        let messageItem = {
                            from: name,
                            phone: phoneNumber,
                            content: await lastestResponse[0].getText(),
                            sentAt: await sendTime[0].getText() + ' ' + day,
                        }
                        responses.push(messageItem);
                        const message = new Message(messageItem);
                        message.save(function (err) {
                            if (!err) console.log('Successfully save message from: ' + messageItem.from);
                        });
